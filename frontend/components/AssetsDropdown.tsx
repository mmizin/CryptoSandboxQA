...
const menuLinkClass =
  'block px-4 py-2 text-sm text-slate-300 hover:bg-slate-800 hover:text-white group-data-[theme=light]:text-slate-700 group-data-[theme=light]:hover:bg-slate-100';
...
export function AssetsDropdown() {
  ...
  const guardNav = (e: React.MouseEvent<HTMLAnchorElement>) => {
    if (!loading && !user) {
      e.preventDefault();
      setOpen(false);
      router.push('/login');
