...
- **Embedded document:** `/qa/iframe-form.html` (static file under `frontend/public/qa/`)
...
Selenium / Cypress equivalents: switch to the frame (or use frame-scoped queries), then locate by the same `data-testid` or by label (`Email`, `Amount (USD)`, etc.).
