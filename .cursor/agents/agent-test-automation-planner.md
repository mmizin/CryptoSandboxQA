...
2. **Ground in the repo** — Read or skim [ARCHITECTURE.md](../../ARCHITECTURE.md) and relevant `.cursor/rules/` for **`tests/backend_tests/`**, **`tests/ui-tests/`**, Playwright tags ([`playwright-ui-tests.mdc`](../rules/playwright-ui-tests.mdc)), **pytest markers** ([`pytest-backend-api-tests.mdc`](../rules/pytest-backend-api-tests.mdc)), layout, and parallelism expectations. If chunks should call out **Allure** layers (epic/feature, steps) for reports, reference [.cursor/skills/allure-reporting/SKILL.md](../skills/allure-reporting/SKILL.md) in the chunk text—only when reporting is in scope.
...
9. **Notes for the implementer** — Pointers to [ARCHITECTURE.md](../../ARCHITECTURE.md), `.cursor/rules/playwright-ui-tests.mdc`, `.cursor/rules/pytest-backend-api-tests.mdc`, `.cursor/rules/test-scenario-conventions.mdc`, `no-unrequested-tests` policy (tests only when in Jira/user scope), the matching **test agent** doc under `.cursor/agents/`, **helper placement rule** (small one-file helpers stay local; promote to shared `utils` only for real cross-module reuse), and **layer-hinted unique test data** (API vs integration vs UI prefixes plus timestamp or random suffix on emails/names/identifiers—see **Unique test data (layer hints)** in those agent files).
