...
**Commits:** By default the assistant does **not** commit when finishing tasks ([.cursor/rules/git-flow.mdc](.cursor/rules/git-flow.mdc)). If there are uncommitted changes, **commit** (locally or after asking the assistant to commit) **before** push/PR, or include that explicit step when the user wants the assistant to prepare the branch for PR.
