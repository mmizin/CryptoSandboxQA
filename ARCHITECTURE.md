...
| `tests/backend_tests/` | Python API test scaffolding (pytest-oriented); **`src/models/user/`** holds dataclass shapes aligned with [`tests/ui-tests/src/models/user/user.types.ts`](tests/ui-tests/src/models/user/user.types.ts) (`UserProfile`, `UserWithProfile`, `UserWithProfileTestData`, `RegisteredTestUser`, etc.); **`src/builders/`** holds **`UserBuilder`** (test payloads, aligned with [`tests/ui-tests/src/builders/user.builder.ts`](tests/ui-tests/src/builders/user.builder.ts)); **`src/services/auth_client.py`** sync **`httpx`** client for `POST /auth/register-with-profile` and `POST /auth/admin/register` (`registration_dict_from_test_data` matches `RegisterWithProfileDto`; `raise_for_status_with_body` surfaces response text on errors); **`src/strategies/user/`** holds **`UserCreationStrategy`**, **`ApiUserCreationStrategy`**, **`AdminApiUserCreationStrategy`** (same roles as Playwright; admin strategy subclasses API strategy); **`src/factories/user_factory.py`** composes **`UserBuilder`** + strategy (strategy-first, like `user.factory.ts`); root **`.env`** / **`.env.example`** supply **`API_URL`** and **`ADMIN_API_KEY`** for the client; **`pyproject.toml`** sets pytest **`pythonpath`** to **`src`** and declares **`httpx`** |
