...
| `.cursor/commands/` | Cursor command playbooks; pipeline flows use a **`cmd_`** filename prefix (e.g. [cmd_qa-test-pipeline.md](.cursor/commands/cmd_qa-test-pipeline.md) — [QA test automation pipeline](#qa-test-automation-pipeline); [cmd_update-knowledge.md](.cursor/commands/cmd_update-knowledge.md) — [LLM wiki knowledge refresh](#llm-wiki-knowledge-refresh)) |
...
**Single command:** [.cursor/commands/cmd_qa-test-pipeline.md](.cursor/commands/cmd_qa-test-pipeline.md) (logical name **qa-test-pipeline**; pipeline-style Cursor commands use the **`cmd_`** filename prefix). Invoke it with the **Story key or Jira Story URL** in the same message; the assistant follows that file end-to-end (not a standalone shell executable). Use a **feature branch** for any repo edits during the pipeline ([.cursor/rules/git-flow.mdc](.cursor/rules/git-flow.mdc)).
...
**Single command:** [.cursor/commands/cmd_update-knowledge.md](.cursor/commands/cmd_update-knowledge.md) (logical name **update-knowledge**). Invoke it in Cursor in the same message as any **scope** (base ref, paths, ticket); the assistant follows that playbook (not a standalone shell executable). The command file defines **source-of-truth order** and the step-by-step procedure.
