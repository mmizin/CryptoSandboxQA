...
| `tests/backend_tests/` | Python API test scaffolding (pytest-oriented); **`framework/models/user/`** holds dataclass shapes aligned with [`tests/ui-tests/src/models/user/user.types.ts`](tests/ui-tests/src/models/user/user.types.ts) (`UserProfile`, `UserWithProfile`, `UserWithProfileTestData`, etc.); **`pyproject.toml`** sets pytest **`pythonpath`** to this folder for imports like `framework.models.user` |
