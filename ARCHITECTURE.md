...
| `tests/ui-tests/` | Playwright tests; specs organized under **`tests/ui-tests/tests/features/<feature>/`** (feature-based structure: `auth/`, `deposits/`, `orders/`, `portfolio/`, etc.) for journeys / flows, and **`tests/ui-tests/tests/unit/`** for narrow UI (e.g. validation matrices)—see [.cursor/rules/playwright-ui-tests.mdc](.cursor/rules/playwright-ui-tests.mdc) for tags vs folders; **`playwright.config.ts`** loads repo **root `.env`** then **`tests/ui-tests/.env`** (overrides); root `.env.example` has `API_URL` / `ADMIN_API_KEY`; **`tests/ui-tests/.env.example`** has **`PLAYWRIGHT_BASE_URL`** / optional seeds; **`src/services/`** holds REST clients (`*.api.ts`, e.g. `AuthApi`, `UserApi`, **`DepositsApi`** for `POST /deposits/fiat` when funding wallets in trade e2e); **`src/pages/`** page objects include **`login.page`**, **`dashboard.page`**, **`trade.page`** (spot/futures order entry, stop orders UI), **`depositAssets.page`** (`/assets/deposit` cash/crypto panes); **`src/models/`** barrel (**`index.ts`**) re-exports domain types and **`user/TestUser`**, **`user/AdminUser`** (Playwright API actors); other domain folders are **`market/`**, **`balances/`**, **`trading/`**, **`payments/`**; strategies include `ApiUserCreationStrategy` and `AdminApiUserCreationStrategy` (`ADMIN_API_KEY`) |
| `tests/backend_tests/` | Python API test scaffolding (pytest-oriented); **`tests/api/`** pytest matrices (e.g. **`tests/api/orders/`** — Orders — KAN-53; **`test_deposits_api.py`** — Deposits — KAN-66); **`conftest.py`** (package root) sets **`pytest_plugins`** to load modules under **`src/plugins/`** (e.g. **`fxt_admin_user`** → **`AdminRegisteredTestUser`** via **`AdminApiUserCreationStrategy`**; **`fxt_regular_user`** → **`RegisteredTestUser`** via **`ApiUserCreationStrategy`**); **`src/models/user/`** holds dataclass shapes aligned with [`tests/ui-tests/src/models/user/user.types.ts`](tests/ui-tests/src/models/user/user.types.ts) (`UserProfile`, `UserWithProfile`, `UserWithProfileTestData`, `RegisteredTestUser`, **`AdminRegisteredTestUser`** for admin bootstrap, etc.); **`src/models/trading/`** holds order types (**`CreateOrderRequest`**, **`Order`**, **`OrdersListResponse`**) aligned with Nest orders and [`tests/ui-tests/src/models/trading/order.types.ts`](tests/ui-tests/src/models/trading/order.types.ts); **`src/models/payments/`** holds deposit types (**`DepositFiatRequest`**, **`DepositCryptoRequest`**, **`DepositFiat`**, **`DepositCrypto`**, list and create-result wrappers) aligned with Nest `DepositsService` and [`tests/ui-tests/src/models/payments/deposit-withdrawal.types.ts`](tests/ui-tests/src/models/payments/deposit-withdrawal.types.ts); **`src/builders/`** holds **`UserBuilder`** (test payloads, aligned with [`tests/ui-tests/src/builders/user.builder.ts`](tests/ui-tests/src/builders/user.builder.ts)), **`OrderBuilder`** (fluent create-order payloads), and **`DepositBuilder`** (fiat/crypto deposit payloads); **`src/factories/order_factory.py`** **`OrderRequestFactory`** presets (spot vs futures × market/limit × buy/sell); **`src/factories/deposit_factory.py`** **`DepositRequestFactory`** presets (fiat card/SEPA, crypto); **`src/services/base_client.py`** shared **`httpx`** **`BaseClient`** (URL resolution, client lifecycle; **`get`/`post`/`put`/`patch`/`delete`** delegate to **`_request`**, which applies **`raise_for_status_with_body`** + **`response.json()`** unless **`expected_failure`** returns the raw **`httpx.Response`**); **`src/services/auth_client.py`** **`AuthClient`** extends **`BaseClient`** for unauthenticated bootstrap: `POST /auth/register-with-profile` and `POST /auth/admin/register` (`registration_dict_from_test_data` matches `RegisterWithProfileDto`); **`src/services/user_client.py`** **`UserClient`** (Bearer JWT) and **`AdminClient`** (subclass of **`UserClient`**) add authenticated user routes plus admin helpers: **`list_users`** (`GET /users`) and **`admin_*`** methods for **`GET /admin/users/:userId/...`** (orders, deposits, wallets, payment-methods, portfolio, transactions), with typed responses where models exist (aligned with **`UserApi`** / **`AdminApi`** in UI tests); **`src/services/orders_client.py`** **`OrdersClient`** (user **`/orders`** plus **`/orders/by-date`** and **`/orders/by-coin`**; shared httpx session via **`UserClient.orders`**); **`src/services/deposits_client.py`** **`DepositsClient`** (user **`/deposits`** routes, shared httpx session via **`UserClient.deposits`**); factories **`user_client_from_registered`** / **`admin_client_from_registered`** build clients from registration results; **`RegisteredTestUser.api`** (lazy **`UserClient`**) / **`AdminRegisteredTestUser.api`** (lazy **`AdminClient`**), same as **`user_client_from_registered`** / **`admin_client_from_registered`**; test call sites use chained **`user.api`** / **`admin.api`** (e.g. `user.api.deposits.deposit_fiat(...)`, `user.api.orders.create(...)`)—do not assign **`api = user.api`**; use the factory helpers directly only when custom client kwargs are needed; **`src/strategies/user/`** holds **`UserCreationStrategy`**, **`ApiUserCreationStrategy`**, **`AdminApiUserCreationStrategy`** (same roles as Playwright; admin strategy subclasses API strategy and returns **`AdminRegisteredTestUser`**); **`src/factories/user_factory.py`** composes **`UserBuilder`** + strategy (strategy-first, like `user.factory.ts`); tests should consume this via injected fixtures and `configure_*` callbacks rather than creating per-spec `UserFactory()` blocks; root **`.env`** / **`.env.example`** supply **`API_URL`** and **`ADMIN_API_KEY`** for the client; **`src/utils/env_loader.py`** loads repo root **`.env`** then optional **`tests/backend_tests/.env`** (overrides) using **`python-dotenv`** before clients read **`os.environ`**; **`pyproject.toml`** lists Python dependencies (**`httpx`**, **`python-dotenv`**, **`pytest`**); **`pytest.ini`** configures discovery (**`pythonpath`**, **`testpaths`**) and **registers pytest markers** (same **@…** run vocabulary as Playwright `tag`—[pytest-backend-api-tests.mdc](.cursor/rules/pytest-backend-api-tests.mdc)) |
...
| **OrdersModule** | Limit/market/**stop** orders (**spot** and **futures** `marketType` in schema), cancel/list; **MatchingService** (FIFO-style matching, trades). Open orders **lock** funds in `user_balances.balance_locked` (sell: base qty; buy: quote ≈ qty × limit price, or **market buy**: qty × last price at submit, stored on `orders.price` for reservation math); **order_lock** / **order_unlock** in `balance_transactions`. Fills settle via locked funds (`WalletsService.settle*InTx`). **Stop orders** use `stopPrice` field (Prisma: `orders.stop_price`); trigger logic implemented in frontend (`lib/orderTriggers.ts`, `lib/useOrderTriggers.ts`) — Buy Stop triggers when price ≥ stopPrice, Sell Stop when price ≤ stopPrice. |
...
- **Static spec**: [openapi.json](openapi.json) (repo root) for offline tools; regenerate via `npm run openapi:generate` from repo root. Response **examples** (JSON/text) are maintained in [`backend/src/openapi/`](backend/src/openapi/) (`api-json-example.decorator.ts`, `response-examples.ts`) and emitted into the generated spec.
...
**In Cursor:** From an existing **Jira Story**, run **test design → Jira test issues → automation plan chunks → test implementation** in one assistant session (ordered phases; parallel test agents only when the plan index allows it).

| Step | What | Repo entry |
|------|------|------------|
| 1 | Structured test cases (API / UI / Integration), Jira-handoff shape | [.cursor/skills/design-test-cases/SKILL.md](.cursor/skills/design-test-cases/SKILL.md) |
| 2 | Create `Test` + `Subtask` issues via Atlassian MCP; link to the Story | [.cursor/skills/jira-test-tickets-from-cases/SKILL.md](.cursor/skills/jira-test-tickets-from-cases/SKILL.md) |
| 3 | Write **`00-index.md`** + **`chunk-NN-….md`** under **`.cursor/plans/<date>-test-automation-<slug>/`**; **Jira** remains authoritative for scope and traceability | [.cursor/agents/agent-test-automation-planner.md](.cursor/agents/agent-test-automation-planner.md) |
| 4 | Execute chunks: parallel **Task** runs where **parallel-safe** and path-disjoint in the index; otherwise sequential | [.cursor/agents/agent-backend-api-tests.md](.cursor/agents/agent-backend-api-tests.md) (`tests/backend_tests/`, API-focused), [.cursor/agents/agent-backend-integration-tests.md](.cursor/agents/agent-backend-integration-tests.md) (multi-step backend journeys), [.cursor/agents/agent-ui-playwright-tests.md](.cursor/agents/agent-ui-playwright-tests.md) (`tests/ui-tests/`) |
| — | **Allure HTML reports** (metadata, steps, hierarchies) when implementing or extending tests | [.cursor/skills/allure-reporting/SKILL.md](.cursor/skills/allure-reporting/SKILL.md) — pytest uses `allure-pytest` + `pytest.ini` `addopts`; Playwright uses `allure-playwright` + `allure-js-commons` |

**Single command:** [.cursor/commands/cmd-qa-test-pipeline.md](.cursor/commands/cmd-qa-test-pipeline.md) (logical name **qa-test-pipeline**; pipeline-style Cursor commands use the **`cmd-`** filename prefix). Invoke it with the **Story key or Jira Story URL** in the same message; the assistant follows that file end-to-end (not a standalone shell executable). Use a **feature branch** for any repo edits during the pipeline ([.cursor/rules/git-flow.mdc](.cursor/rules/git-flow.mdc)).

---

## LLM wiki knowledge refresh

**Normal rule:** When you finish **meaningful** product or test-harness work, update **[`ARCHITECTURE.md`](ARCHITECTURE.md)** in the **same change set / PR** as the code if the overview would be wrong or incomplete ([.cursor/rules/project-conventions.mdc](.cursor/rules/project-conventions.mdc)) — do not rely on a later pass by default.

The **`update-knowledge`** playbook mainly **reconciles [`knowledge/llm-wiki/`](knowledge/llm-wiki/README.md)** with the repo (diff vs **`master`** or a base you name): refresh **`knowledge/llm-wiki/wiki/`** maps when “where to look” paths changed, and optionally add notes under **`knowledge/llm-wiki/raw/`**. **Editing `ARCHITECTURE.md` from that command is optional** — use it to **catch drift**, fix a **missed** earlier doc update, or when the user **explicitly** asks to align the architecture section; if the feature PR already updated this file, the playbook often only touches **`wiki/`** (or reports nothing to do).

**Single command:** [.cursor/commands/cmd-update-knowledge.md](.cursor/commands/cmd-update-knowledge.md) (logical name **update-knowledge**). Invoke it in Cursor in the same message as any **scope** (base ref, paths, ticket); the assistant follows that playbook (not a standalone shell executable). The command file defines **source-of-truth order** and the step-by-step procedure.
