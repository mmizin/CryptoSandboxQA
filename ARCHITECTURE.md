...
| `tests/ui-tests/` | Playwright tests; **`playwright.config.ts`** loads repo **root `.env`** then **`tests/ui-tests/.env`** (overrides); root `.env.example` has `API_URL` / `ADMIN_API_KEY`; **`tests/ui-tests/.env.example`** has **`PLAYWRIGHT_BASE_URL`** / optional seeds; **`src/models/`** exposes **`TestUser`** / **`AdminUser`** (API actors) and domain-aligned types under **`user/`**, **`market/`**, **`balances/`**, **`trading/`**, **`payments/`** (barrel: **`src/models/index.ts`**); strategies include `ApiUserCreationStrategy` and `AdminApiUserCreationStrategy` (`ADMIN_API_KEY`) |
