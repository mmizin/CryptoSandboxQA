...
| 11 | Bulk User Import (CSV/JSON) | 2 | Medium | M | ~1d | Admin Role & Guards | PRD | Technical Design (parser format detection — covered directly in PRD Current Behavior/FCs; no separate architecture artifact needed) | Depends: PRD-AdminRoleGuards · Produces: docs/features/bulk-user-import/PRD.md (Slug: PRD-BulkUserImport) | **Done** — DoD met: PRD (Slug: PRD-BulkUserImport) passed product-reviewer across a Full Review (9 findings, all fixed — including two sibling-route misattributions conflating `PRD-AdminUserCreation`'s `ADMIN_API_KEY`-gated bootstrap route with the actual, guard-identical single-row counterpart `POST /auth/admin/create-user`, a disproven "same validation rules" parity claim, an understated frontend/backend result-joining risk, and an unasked max-batch request-timeout gap) and two Delta Reviews (7 then 6 further findings, all fixed, converging to Accept on round 3 with one trivial phrasing leftover); external dependencies correctly referenced rather than absorbed — `ADR-0020` (Accepted) OQ-2 and `PRD-AdminRoleGuards` Q-008 are both cited as owning related open questions rather than re-decided here; open questions logged (Q-001–Q-014, including Q-003a), none silently dropped; no separate architecture artifact required — parser/format-detection behavior is fully covered in the PRD's Current Behavior and Failure Conditions sections, consistent with this backlog row's original scope note |
| 12 | Impersonation (Admin → User) | 2 | High | M | ~1.5d | Admin Role, Session Mgmt | PRD | Architecture Discovery, ADR-0021 (Accepted), C4 (confirmed not needed) | Depends: PRD-AdminRoleGuards, PRD-SessionManagement · Produces: docs/features/impersonation/{PRD.md, DISCOVERY.md}, docs/architecture/adr/0021 | **Done** — DoD met: PRD (Slug: PRD-Impersonation) passed product-reviewer (Full Review + Delta Review, all findings fixed); Discovery correctly separated one genuine candidate ADR (Q-001b's stranded-restore-token fallback gap, unowned by both ADR-0011 and ADR-0016) from a plain FR-level fix wrongly first promoted to candidate status (self-impersonation, FC-009/Q-002 — demoted after re-assessment to a single non-branching validation gap, no competing options, stays PRD-owned); candidate ADR-0021 went through 4 Full Reviews, 3 Delta Reviews, and 2 narrow confirmation passes — correcting, across rounds, a briefly-adopted but broken frontend fix (Option D v1, reverted after tracing showed it produced a stale banner/live credential rather than a working retry), a misattributed sibling-ADR routing (ADR-0016's own out-of-scope item conflated with a different PRD-AdminRoleGuards consequence), an unreconciled contradiction with PRD-AdminRoleGuards G-002, and a missing realistic option in the final option set — before reaching Accepted on Option A (no server-side change; forced re-authentication accepted when a demoted admin's restore is correctly refused), with a genuinely cheaper alternative (Option D v2: suppress only the logout, no retry) recorded as deliberately deferred rather than adopted or discarded; c4-expert confirmed no C4 diagram warranted (no new container/component boundary, consistent with prior features); six follow-up documentation actions logged directly in ADR-0021 with named owners (4× `PRD-Impersonation`, 1× `PRD-AdminRoleGuards`, 1× `architecture-librarian`) — tracked separately as documentation maintenance, not blocking this feature's completion |
| 13 | Admin User List & Query | 2 | Low | S | ~0.5d | Admin Role & Guards | PRD | — | Depends: PRD-AdminRoleGuards · Produces: docs/features/admin-user-list-query/PRD.md (Slug: PRD-AdminUserList) | **Done** — DoD met: PRD (Slug: PRD-AdminUserList) reached Approved after 3 Full Reviews + 1 Delta Review; Full Review 1 caught a wrongly-excluded search capability (the draft read only the inventory's parameter table and missed its own Purpose line, "Admins can list and search registered users"); Full Review 2 found the deeper problem — the entire document rested on `FEATURES_INVENTORY.md`'s entry for this feature, which misdescribes the shipped endpoint (claims pagination and a 7-field paginated envelope; the real `GET /users?search=` endpoint has no pagination, returns a bare 4-field array `{id, email, displayName, role}`, hard-capped at 100 rows, sorted `email` ascending) — the document was rewritten wholesale from source per explicit user direction, the same correction precedent `PRD-Impersonation` had already set for a different inventory divergence; Full Review 3 confirmed the rewrite accurate against source but found the "list" (no-search-term) half of the feature — the confirmed consumer's actual default state — had no Goal/User Story/AC coverage, plus 5 smaller gaps (a confident-failure gap where the one real consumer, `PRD-Impersonation`'s account picker, degrades a request failure to the same empty result as "no matches"; an Open Question overstating its own citation; a fused security-requirement/projection-freeze FR; an inert Assumptions section; an NFR section pointing at a question that didn't exist) — all fixed in a bounded pass; Delta Review confirmed all 8 findings resolved and surfaced 2 more small residuals (the "no more than 100 users" assumption not accounting for `PRD-BulkUserImport`'s 500-user single-request import — elevated Q-002 toward blocking; an unfixed AC left non-mechanically-checkable by the FR split) — both fixed directly per owner decision to skip a further paid review round; open questions logged (Q-001–Q-007), none silently dropped; confirmed blocking outbound integration with `PRD-Impersonation` (its account picker is this feature's one real consumer, tying its compatibility to this feature's response shape); no architecture artifacts required per backlog |
